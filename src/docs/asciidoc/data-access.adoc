...
Aa0Application code that must execute in a transactional context and that explicitly uses the
...
=== Aa0Application server-specific integration
...
which is available in WebSphere Aa0Application Server 6.1.0.9 and later. With this adapter,
...
Aa0Application code is required to retrieve the JDBC connection through
...
Spring container. Aa0Application objects that need to access resources receive references
...
==== Spurious Aa0Application Server Warnings with Hibernate
...
----
	...
	public class Aa0Application {
		...
		public static void main(String[] args) throws IOException {
			...
			Aa0Application application = (Aa0Application) appContext.getBean("application");
...
----
	...
	fun main(args: Array<String>) {
		...
		val application = appContext.getBean("application") as Aa0Application
		application.saveSettings()
		application.loadSettings()
	}
----

The `Aa0Application` requires both a `marshaller` and an `unmarshaller` property to be set. We
can do so by using the following `applicationContext.xml`:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<beans>
		<bean id="application" class="Aa0Application">
