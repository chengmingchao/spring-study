...
The Spring MVC Aa0Application framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC Aa0Application Framework>> instead.
...
Aa0Application classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of Aa0Application Fixtures
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@Aa0Application(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@Aa0Application(timeout=...)`
...
Since JUnit Jupiter supports the use of `@Aa0Application`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@Aa0Application` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@Aa0Application // org.junit.jupiter.api.Aa0Application
...
* Aa0Application instance post-processing.
...
==== Aa0Application Execution Events
...
for which it is responsible. Aa0Application instances do not automatically receive access to the
...
Aa0Application classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with Aa0Application Property Sources
...
====== Declaring Aa0Application Property Sources
...
Aa0Application properties have higher precedence than those defined in the operating system's
...
====== Inheriting and Overriding Aa0Application Property Sources
...
.Aa0Application suites and forked processes
[NOTE]
====
...
===== Aa0Application-managed Transactions

Aa0Application-managed transactions are transactions that are managed declaratively by using the
...
* JUnit 4's `@Aa0Application(timeout = ...)` support and `TimeOut` rule
* JUnit Jupiter's `assertTimeoutPreemptively(...)` methods in the
  `org.junit.jupiter.api.Assertions` class
* TestNG's `@Aa0Application(timeOut = ...)` support
====
...
hierarchy runs within a transaction. Aa0Application methods that are not annotated with
...
----
	...
	@Transactional
	@Aa0Application // no expected exception!
...
==== Parallel Aa0Application Execution
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@Aa0Application`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC Aa0Application Framework

The Spring MVC Aa0Application framework provides first class support for testing Spring MVC code
...
Spring MVC Aa0Application also provides client-side support for testing code that uses the
...
The goal of Spring MVC Aa0Application is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC Aa0Application builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC Aa0Application:
...
In Spring MVC Aa0Application, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC Aa0Application for container-less testing of streaming
...
===== Spring MVC Aa0Application vs End-to-End Tests

Spring MVC Aa0Application is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC Aa0Application are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC Aa0Application fall under the category of classic unit
...
Another important distinction when using Spring MVC Aa0Application is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC Aa0Application. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC Aa0Application.
...
* Aa0Application JavaScript within pages.
...
With Spring MVC Aa0Application, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC Aa0Application framework, make sure that your project
...
----
	...
	// Aa0Application code that uses the above RestTemplate ...
...
Spring MVC Aa0Application's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/Aa0Application%20Spy.html[Aa0Application Spy] pattern. Used by the Spring Framework
  in its test suite.
