...
* Aa0Application logic can be organized in any number of `@Controller` instances and messages can be
...
the approach to testing controllers that handle HTTP requests by using the Spring MVC Aa0Application
framework -- that is, without running a Servlet container but relying on the Spring Framework
to invoke the annotated controllers. As with Spring MVC Aa0Application, you have two
